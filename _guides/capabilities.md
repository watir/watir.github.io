...
Note that you can combine W3C and Browser specific keys in the same `:options` Hash:
{% highlight ruby %}
browser_opts = {accept_insecure_certs: true,
                timeouts: {page_load: 100,
                           script: 30},
...
To route commands via a server or service, simply pass in the location of the
...
To run on a local machine, Watir uses Selenium code to determine the correct
[installed driver](../drivers) associated with the provided `:browser_name`, starts the driver,
and sets it to be the recipient of the commands. You can customize how the 
...
An example of setting service parameters would be:
{% highlight ruby %}
...
The WebDriver protocol is based on using an HTTP client to send JSON packets to an endpoint.
As of Watir 6.19, we default to using our own custom client: `Watir:HttpClient`. 
All it is doing differently is adding some header information to 
indicate to Selenium service providers that Watir is being used and including the version. 
...
what you'd like to see us add in ours.
...
{% highlight ruby %}
b = Watir::Browser.new :chrome, options: {headless: true}
{% endhighlight %}
or
{% highlight ruby %}
b = Watir::Browser.new :firefox, options: {headless: true}
{% endhighlight %}
...
[comment]: <> (TODO: Create a Safari Page to discuss the preview and move this description over)
This tests the beta version of Safari, but because the driver for Safari is released
much less infrequently than the technology preview, and Safari lags behind the other
drivers in its implementation, you are likely to be able to test more things with
this setting turned on, than otherwise. Just make sure you
[download it](https://developer.apple.com/safari/technology-preview/) first.
{% highlight ruby %}
b = Watir::Browser.new :safari, options: {technology_preview: true}
{% endhighlight %}
...
#### Capabilities
Note: You can use Capabilities, but it is better to use the `Options` class if at all possible
{% highlight ruby %}
